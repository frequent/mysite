...
(function (window, document, RSVP, rJS, XMLHttpRequest, URL,
  ...
  function parseSocialMediaElement(social_media_element) {
    var ul = social_media_element.querySelector('ul'),
      social_media_list = [],
      li_list,
      link,
      i;
    ...
    for (i = 0; i < li_list.length; i += 1) {
      link = li_list[i].querySelector('a');
      social_media_list.push({
        href: link.href,
        title: link.title,
        icon_class_string: link.firstChild.getAttribute("class")
      }); 
    }
  ...
  function parsePageContent(body_element, language, alternate_element,
    ...
    return {
      ...
      footer_html_content: body_element.querySelector('footer').innerHTML,
      language: language,
      ...
      social_media_list: parseSocialMediaElement(
        body_element.querySelector('nav#socialmedia')
      ),
